...
#   Import sub-methods.
from gnomics.objects.taxon_files.accepted_names import get_accepted_names
...
from gnomics.objects.taxon_files.paleobiology_db import get_paleobiology_db_id
from gnomics.objects.taxon_files.scientific_name import get_scientific_name
from gnomics.objects.taxon_files.search import search
from gnomics.objects.taxon_files.vernacular_names import get_vernacular_names
from gnomics.objects.taxon_files.vto import get_vto_id
from gnomics.objects.taxon_files.wiki import get_wikidata_object

#   Import further methods.
from gnomics.objects.interaction_objects.taxon_phenotype import get_phenotypes
from gnomics.objects.interaction_objects.taxon_reference import get_references
from gnomics.objects.interaction_objects.taxon_tissue import get_tissues
...
class Taxon(object):
    """
        ...
        populations which are reproductively isolated from other such groups." However, in practice, this 
        class contains any taxonomical entity, as long as 
        that entity is biological in nature (i.e. viruses, 
        etc.). Note that this class does not cover 
        chemical species.
        
    """
    
    # ATO BioPortal PURL.
    ato_bioportal_purl = "http://purl.bioontology.org/ontology/ATO"
    
    # FB-SP BioPortal PURL.
    fbsp_bioportal_purl = "http://purl.bioontology.org/ontology/FB-SP"
    
    # NCBITAXON BioPortal PURL.
    ncbitaxon_bioportal_purl = "http://purl.bioontology.org/ontology/NCBITAXON"
    
    # VTO BioPortal PURL.
    vto_bioportal_purl = "http://purl.bioontology.org/ontology/VTO"
    ...
    # Add an object to a taxon.
    def add_object(taxon, obj=None, object_type=None):
    ...
    def all_identifiers(taxon, user = None):
        Taxon.accepted_names(taxon, user = user)
        Taxon.col_id(taxon, user = user)
        Taxon.eol_id(taxon, user = user)
        Taxon.itis_tsn(taxon, user = user)
        Taxon.kegg_species_id(taxon, user = user)
        Taxon.ncbi_taxonomy_id(taxon, user = user)
        Taxon.paleobiology_db_id(taxon, user = user)
        Taxon.scientific_name(taxon, user = user)
        Taxon.vernacular_names(taxon, user = user)
        Taxon.vto_id(taxon, user = user)
        return taxon.identifiers
    
    # Get accepted names.
    def accepted_names(taxon, user = None):
        return get_accepted_names(taxon)
    
    # Get COL (Cataologue of Life) identifier.
    def col_id(taxon, user = None):
        return get_col_id(taxon)
    
    # Get EOL (Encyclopedia of Life) identifier.
    def eol_id(taxon, user = None):
        return get_eol_id(taxon)
        
    # Get ITIS Taxonomic Serial number.
    def itis_tsn(taxon, user = None):
        print("NOT FUNCTIONAL.")
    
    # Get KEGG species identifier.
    def kegg_species_id(taxon, user = None):
        print("NOT FUNCTIONAL.")
    
    # Get NCBI Taxonomy identifier.
    def ncbi_taxonomy_id(taxon, user = None):
        return get_ncbi_taxonomy_id(taxon)
    
    # Get Paleobiology Database identifier.
    def paleobiology_db_id(taxon, user = None):
        return get_paleobiology_db_id(taxon)
        
    # Get scientific name.
    def scientific_name(taxon, user = None):
        return get_scientific_name(taxon)
    
    # Get vernacular names.
        # TODO: Make a way to request by language?
    def vernacular_names(taxon, user = None):
        return get_vernacular_names(taxon)
    
    # Get VTO ID.
    def vto_id(taxon, user = None):
        return get_vto_id(taxon)
    ...
    def all_interaction_objects(taxon, user = None):
        interaction_obj = {}
        interaction_obj["Phenotypes"] = Taxon.phenotypes(taxon, user=user)
        interaction_obj["References"] = Taxon.references(taxon, user=user)
        interaction_obj["Tissues"] = Taxon.tissues(taxon, user=user)
        return interaction_obj
    
    # Return phenotypes.
    def phenotypes(taxon, user=None):
        return get_phenotypes(taxon)
    
    # Return references.
    def references(taxon, user=None):
        return get_references(taxon)
    
    # Return tissues.
    def tissues(taxon, user=None):
        return get_tissues(taxon)
    ...
    def age_at_eye_opening(taxon):
        trait_array = []
        for obj in Taxon.eol_traitbank(taxon):
            for trait in obj["item"]["traits"]:
    ...
    def search(query, source = "eol", page = 1, exact = True, filter_by_taxon_concept_id = "", filter_by_hierarchy_entry_id = "", filter_by_string = "", cache_ttl = "", user = None):
        return search(query, source = source, page = page, exact = exact, filter_by_taxon_concept_id = filter_by_taxon_concept_id, filter_by_hierarchy_entry_id = filter_by_hierarchy_entry_id, filter_by_string = filter_by_string, cache_ttl = cache_ttl, user = user)
