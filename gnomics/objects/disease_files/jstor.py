...
#   Get JSTOR Topic ID.
def get_jstor_topic_id(dis):
    dis_array = []
    
    for ident in dis.identifiers:
        if ident["identifier_type"].lower() in ["jstor", "jstor id", "jstor identifier", "jstor topic", "jstor topic id", "jstor topic identifier"]:
                    ...
                    if en_prop_name.lower() == "jstor topic id":
                        for x in prop_dict:
                            if x["mainsnak"]["datavalue"]["value"] not in dis_array:
                                gnomics.objects.disease.Disease.add_identifier(dis, identifier=x["mainsnak"]["datavalue"]["value"], identifier_type="JSTOR Topic ID", language=None, source="Wikidata")
