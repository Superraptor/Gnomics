...
#   Get NDL Auth ID.
def get_ndl_auth_id(dis):
    dis_array = []
    
    for ident in dis.identifiers:
        if ident["identifier_type"].lower() in ["ndl", "ndl auth id", "ndl auth identifier"]:
        ...
        if ident["identifier_type"].lower() in ["wikidata", "wikidata id", "wikidata identifier", "wikidata accession"]:
            for stuff in gnomics.objects.disease.Disease.wikidata(dis):
                for prop_id, prop_dict in stuff["claims"].items():
                    ...
                    if en_prop_name.lower() == "ndl auth id":
                        for x in prop_dict:
                            if x["mainsnak"]["datavalue"]["value"] not in dis_array:
                                gnomics.objects.disease.Disease.add_identifier(dis, identifier=x["mainsnak"]["datavalue"]["value"], identifier_type="NDL Auth ID", language=None, source="Wikidata")
