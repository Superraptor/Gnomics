...
#   MAIN
def main():
    emedicine_unit_tests()

#   Get GND ID.
def get_emedicine_id(dis):
    dis_array = []
    for iden in gnomics.objects.auxiliary_files.identifier.filter_identifiers(dis.identifiers, ["emedicine", "emedicine id", "emedicine identifier"]):
        ...
        if ident["identifier_type"].lower() in ["wikidata", "wikidata id", "wikidata identifier", "wikidata accession"]:
            for stuff in gnomics.objects.disease.Disease.wikidata(dis):
                for prop_id, prop_dict in stuff["claims"].items():
                    ...
                    if en_prop_name.lower() == "emedicine":
                        for x in prop_dict:
                            if x["mainsnak"]["datavalue"]["value"] not in dis_array:
                                gnomics.objects.disease.Disease.add_identifier(dis, identifier = x["mainsnak"]["datavalue"]["value"], identifier_type = "eMedicine ID", language = None, source = "Wikidata")
