...
#   MAIN
def main():
    patientplus_unit_tests()

#   Get PatientPlus ID.
def get_patientplus_id(dis):
    dis_array = []
    
    for ident in dis.identifiers:
        if ident["identifier_type"].lower() in ["patientplus", "patientplus id", "patientplus identifier"]:
        ...
        if ident["identifier_type"].lower() in ["wikidata", "wikidata id", "wikidata identifier", "wikidata accession"]:
            for stuff in gnomics.objects.disease.Disease.wikidata(dis):
                for prop_id, prop_dict in stuff["claims"].items():
                    ...
                    if en_prop_name.lower() == "patientplus id":
                        for x in prop_dict:
                            if x["mainsnak"]["datavalue"]["value"] not in dis_array:
                                gnomics.objects.disease.Disease.add_identifier(dis, identifier=x["mainsnak"]["datavalue"]["value"], identifier_type="PatientPlus ID", language=None, source="Wikidata")
