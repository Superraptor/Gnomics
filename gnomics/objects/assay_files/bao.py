...
def get_bao_id(assay):
    bao_array = []
    for iden in gnomics.objects.auxiliary_files.identifier.filter_identifiers(assay.identifiers, ["bao", "bao id", "bao identifier"]):
        if iden["identifier"] not in bao_array:
            bao_array.append(iden["identifier"])
    return bao_array
...
def bao_unit_tests(bioassay_id, bao_id, openphacts_app_id, openphacts_app_key):
    ...
    for iden, val in get_bao_class_assays(bao_assay, user = user, count_only = False).items():
        for x in val:
            for fier in x.identifiers:
                print("- %s (%s)" % (fier["identifier"], fier["identifier_type"]))  
