...
#
#   Get biological processes from pathway.
#
...
#   MAIN
def main():
    pathway_biological_process_unit_tests("ko00270")
     
#   Get biological processes.
def get_biological_processes(pathway, user=None):
    molec_array = []
    molec_id_array = []
                        ...
                        if "GO" in temp["DBLINKS"]:
                            for temp_go in temp["DBLINKS"]["GO"].split(" "):
                                ...
                                if not r.ok:
                                    ...
                                    decoded = r.json()
                                    if decoded["namespace"] == "biological_process":
                                        go_acc = decoded["accession"]
                                        go_name = decoded["name"]
                                        temp_molec = gnomics.objects.biological_process.BiologicalProcess(identifier=go_acc, identifier_type="GO Accession", language=None, source="Ensembl", name=go_name)
...
#   UNIT TESTS
def pathway_biological_process_unit_tests(kegg_ko_pathway_id):
    ...
    for molec in get_biological_processes(kegg_ko_pathway):
