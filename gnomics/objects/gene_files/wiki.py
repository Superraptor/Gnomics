...
#   MAIN
def main():
    wiki_unit_tests("ENSG00000012048")
...
def get_arabic_wikipedia_accession(gene, user = None):
    ...
    if wiki_array:
        return wiki_array
    for ident in gene.identifiers:
        if ident["identifier_type"].lower() == "ensembl" or ident["identifier_type"].lower() == "ensembl id" or ident["identifier_type"].lower() == "ensembl identifier" or ident["identifier_type"].lower() == "ensembl gene identifier" or ident["identifier_type"].lower() == "ensembl gene id":
...
def get_estonian_wikipedia_accession(gene, user = None):
    wiki_array = []
    for ident in gene.identifiers:
        if (ident["identifier_type"].lower() == "wikipedia accession" or ident["identifier_type"].lower() == "wikipedia") and ident["language"] == "et":
            wiki_array.append(ident["identifier"])
    if wiki_array:
        return wiki_array
    for ident in gene.identifiers:
...
def get_urdu_wikipedia_accession(gene, user = None):
    wiki_array = []
    for ident in gene.identifiers:
        if (ident["identifier_type"].lower() == "wikipedia accession" or ident["identifier_type"].lower() == "wikipedia") and ident["language"] == "ur":
            wiki_array.append(ident["identifier"])
    if wiki_array:
        return wiki_array
    for ident in gene.identifiers:
...
def get_wikidata_object(gene):
    ...
    for gene_obj in gene.gene_objects:
        if 'object_type' in gene_obj:
            if gene_obj['object_type'].lower() == 'wikidata object' or gene_obj['object_type'].lower() == 'wikidata':
...
#   UNIT TESTS
def wiki_unit_tests(ensembl_gene_id):
