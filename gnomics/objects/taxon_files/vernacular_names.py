...
#   MAIN
def main():
    vernacular_name_unit_tests("1045608", eol_api_key = "")
    
def get_vernacular_names(taxon):
    vernacular_name_array = []
    
    for ident in taxon.identifiers:
        if ident["identifier_type"].lower() in ["vernacular name", "common name"] and ident["identifier"] not in vernacular_name_array:
    ...
    for ident in taxon.identifiers:
        if ident["identifier_type"].lower() in ["eol", "eol id", "eol identifier"]:
            for eol_obj in gnomics.objects.taxon.Taxon.eol_page(taxon):
